...
def analyze_correlations(df: pd.DataFrame, available_measures: list, input_type: str, task_name: str):
    ...
    corr_matrix = df_filtered[ordered_cols].corr(method='spearman')
    ...
    _add_side_colorbar(ax, hm.collections[0], label="Spearman Correlation")
    _finalize_ax(ax, title=f"Global Spearman ({task_name.title()})")
...
def analyze_conditional_correlations_sr(df: pd.DataFrame, available_measures: list, input_type: str, task_name: str, storing: str):
    ...
    for name, (low, high) in SPECTRAL_RANGES.items():
        ...
        corr_matrix = range_df_used[valid_cols].corr(method='spearman')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(storing, f"5_spearman_correlation_by_sr_{task_name}.pdf"), bbox_inches="tight", dpi=300)
    plt.close()
...
def analyze_correlations_by_input_count(df: pd.DataFrame, available_measures: list, input_type: str, task_name: str, storing: str):
    ...
    for count in sorted(df['process_count'].unique()):
        ...
        corr_matrix = sub_used[valid_cols].corr(method='spearman')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(storing, f"5_spearman_correlation_by_input_count_{task_name}.pdf"), bbox_inches="tight", dpi=300)
    plt.close()
...
def analyze_correlations_by_scale(df: pd.DataFrame, available_measures: list, input_type: str, task_name: str, storing: str):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(storing, f"5_spearman_correlation_by_scale_{task_name}.pdf"), bbox_inches="tight", dpi=300)
    plt.close()
