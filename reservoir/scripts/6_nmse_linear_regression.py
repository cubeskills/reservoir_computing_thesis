...
def generate_global_regression_plots(df_full: pd.DataFrame, available_measures: list, output_dir: str):
    ...
    csv_path = os.path.join(output_dir, "6_linreg_global_r2_scores.csv")
    ...
    plt.title('Linear Regression Performance (Test R²) with Standard Deviation', fontsize=14)
    plt.xticks(x_pos, results_df.index, rotation=45, ha='right')
    plt.grid(True, axis='y', linestyle='--', alpha=0.6)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "6_linreg_global_r2_barplot.pdf"), dpi=300, bbox_inches='tight')
    plt.close()
