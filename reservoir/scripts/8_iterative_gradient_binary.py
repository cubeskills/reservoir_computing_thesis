...
def plot_error_landscape_with_trajectories(csv_file, task_name, process_count, trajectories, EPOCHS):
    ...
    vmin = float(np.nanmin(pivoted_df.values)) + 1e-9
    vmax = float(np.nanmax(pivoted_df.values))
    ...
    format_colorbar(cbar, "Mean NMSE (log scale)")
...
def main():
    ...
    df_processed = df[df['task'].isin(['parity'])]
    df_processed = df_processed[df_processed['process_count'].isin([1, 3, 7])]
        ...
        for scale_init in SCALE_INITS:
            ...
            for epoch in tqdm(range(EPOCHS), desc="Optimizing Hyperparameters"):
                ...
                process = compute_random_processes(
                    5, n_total_steps=1000,
                    lower=-np.sqrt(3/4), upper=np.sqrt(3/4), seed=epoch
                ).T

                nmse_histories["parity"].append(
                    evaluate_performance("parity", process, current_sr, current_scale, SEED)
                )
        

                meas_reservoir = build_reservoir(
                    input_dim=5, reservoir_size=100,
